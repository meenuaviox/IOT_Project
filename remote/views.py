...
class ObjectSensorMapping(TemplateView):
	...
	def post(self, request, *args, **kwargs):
		print(request.POST,'hhhhhhhhhhhhhhhhhhhh')
		active=request.POST.get("active")
		try:
			if 'username' in request.session:
				user= request.session['username']
				...
				companyid=active_user_info.companyid.companyid
				if active == 'Y':
					objectsensormapping=Objectsensormapping.objects.filter(companyid=companyid,activerecord='Y')
				else:
					objectsensormapping=Objectsensormapping.objects.filter(companyid=companyid,activerecord='N')
			elif request.user.is_staff:
				if active == 'Y':
					objectsensormapping=Objectsensormapping.objects.filter(activerecord='Y')
				else:
					objectsensormapping=Objectsensormapping.objects.filter(activerecord='N')			
		except Exception as e:
			raise e
		return render(request,self.template_name,locals())
	...
	def post(self,request,*args,**kwargs):
		...
		try:
			if 'username' in request.session:
				...
				active_user_info=Usermaster.objects.get(username=user)
				companyid=active_user_info.companyid.companyid
				details=Objectzonedetails.objects.filter(updateddatetime__range=[fromdate, todate])
			else:
				details=Objectzonedetails.objects.filter(updateddatetime__range=[fromdate, todate])
		except Exception as e:
			raise e
		...
		try:
			...
			if 'username' in request.session:
				...
				if filters == "all":
					...
					details=Objectzonedetails.objects.filter(companyid=companyid,zonename=zonename,sensorid=sensorid,objecttypeid=objectype)
			else:
				if filters == "all":
					details=Objectzonedetails.objects.all()
				else:
					details=Objectzonedetails.objects.filter(zonename=zonename,sensorid=sensorid,objecttypeid=objectype)
